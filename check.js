...
function showWaiting(email='',uid=user?.uid){document.getElementById("root").innerHTML=`<div class="registrationWait"><div class="waitbox"><div class="waitBrand"><span>D</span><b>H</b><span>S</span></div><div class="waitPulse"><div class="waitSpinner"></div></div><div class="waitTitle">Waiting for document checking</div><p class="waitSub">Your registration is under review</p><p class="waitEmail">${esc(email)}</p><p class="waitCopy">DHS Admin is checking your registration details and documents. Please keep this page open. It will automatically continue when Admin approves your partner registration.</p><div class="waitProgress"><i></i></div><span class="waitLock">🔒 Secure DHS Partner verification</span></div></div>`;if(uid)listenRegistrationApproval(uid)}
...
function showNewBookingPopup(j){
 if(!online)return;
 if(!j?.id)return;
 if(notifiedJobs.has(String(j.id)) && document.getElementById('np'))return;
 ...
 document.body.insertAdjacentHTML('beforeend',`<div class="notificationPop workGlass" id="np"><h3>🔔 New DHS Booking</h3><div class="jobDetails"><div>Customer: <b>${esc(customer)}</b></div><div>Service: <b>${esc(service)}</b></div><div>Address: <b>${esc(addr)}</b></div></div><div class="actions"><button class="btn green" onclick="jobDecision('${esc(j.id)}','ACCEPTED');document.getElementById('np')?.remove()">Accept</button><button class="btn danger" onclick="jobDecision('${esc(j.id)}','REJECTED');document.getElementById('np')?.remove()">Reject</button></div></div>`);
}
...
function renderEarnings(){let done=jobs.filter(j=>['COMPLETED','Completed'].includes(String(first(j,['partnerStatus','Stuts','status'],''))));document.querySelector('#statJobs').textContent=done.length;document.querySelector('#statPay').textContent=money(done.reduce((s,j)=>s+Number(first(j,['amount','total','payment'],0)),0));document.querySelector('#statBills').textContent=bills.length;document.querySelector('#statAccepted').textContent=jobs.filter(j=>String(first(j,['partnerStatus','Stuts'],'')).toUpperCase()==='ACCEPTED').length;document.querySelector('#completedList').innerHTML=done.map(j=>`<div class="card"><b>${esc(first(j,['costumername','customerName'],'Customer'))}</b><div class="muted">${esc(first(j,['service'],'Service'))} • ${money(first(j,['amount','total'],0))}</div></div>`).join('')||'<div class="card empty">No completed jobs yet.</div>'}
